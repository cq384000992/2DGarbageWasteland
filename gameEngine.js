...
class GameEngine {
    ...
    useSkill1() {
        if (this.gameState.skill1Cooldown <= 0) {
            ...
            // 磁暴弹技能逻辑
            const skillEffect = new SkillEffect(this.gameObjects.player.x, this.gameObjects.player.y, 'magnetic');
            this.gameObjects.particles.push(skillEffect);
        }
    }
    ...
    useSkill2() {
        if (this.gameState.skill2Cooldown <= 0) {
            ...
            // 护盾技能逻辑
            const skillEffect = new SkillEffect(this.gameObjects.player.x, this.gameObjects.player.y, 'shield');
            this.gameObjects.particles.push(skillEffect);
        }
    }
    ...
    update(deltaTime) {
        ...
        // 更新子弹
        this.gameObjects.bullets.forEach(bullet => bullet.update(deltaTime, this.canvas, this.camera));
    ...
    updateUI() {
        ...
        this.uiManager.updatePlayerPosition(this.gameObjects.player);
    ...
    gameOver(isWin) {
        ...
        if (isWin) {
            // 通关时显示当前血量
            const currentHP = Math.max(0, this.gameObjects.player.hp);
            this.uiManager.showGameOver(true, this.gameState.enemiesKilled, currentHP);
        } else {
            // 失败时显示0血量
            this.uiManager.showGameOver(false, this.gameState.enemiesKilled, 0);
        }
